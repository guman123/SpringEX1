...
public class UploadServiceImple implements UploadService {
	...
	public StoreListDomain storeSelectOne(HashMap<String, Object> map) {
		return uploadMapper.storeSelectOne(map);
	}
	...
	public List<StoreFileDomain> storeSelectOneFile(HashMap<String, Object> map) {
		return uploadMapper.storeSelectOneFile(map);
	}
	...
	public int storefileProcess(StoreListVO storeListVO, MultipartHttpServletRequest request, HttpServletRequest httpReq) {
		...
		//content domain 생성 
		StoreContentDomain storeContentDomain = StoreContentDomain.builder()
				.mbId(session.getAttribute("id").toString())
				.stTitle(storeListVO.getTitle())
				.stContent(storeListVO.getContent())
				.build();
		
				if(storeListVO.getIsEdit() != null) {
					storeContentDomain.setStSeq(Integer.parseInt(storeListVO.getSeq()));
					System.out.println("수정업데이트");
					// db 업데이트
					uploadMapper.stContentUpdate(storeContentDomain);
				}else {	
					// db 인서트
					uploadMapper.stcontentUpload(storeContentDomain);
					System.out.println(" db 인서트");

				}
				
				// file 데이터 db 저장시 쓰일 값 추출
				int stSeq = storeContentDomain.getStSeq();
				String mbId = storeContentDomain.getMbId();
				...
				// 게시글 수정시 파일관련 물리저장 파일, db 데이터 삭제 
				if(storeListVO.getIsEdit() != null) { // 수정시 

	
					List<StoreFileDomain> storeList = null;
					...
					for (MultipartFile multipartFile : multipartFiles) {
						...
						if(!multipartFile.isEmpty()) {   // 수정시 새로 파일 첨부될때 세션에 담긴 파일 지우기
							...
							if(session.getAttribute("stfiles") != null) {	

								storeList = (List<StoreFileDomain>) session.getAttribute("stfiles");
								
								for (StoreFileDomain list : storeList) {
									list.getUpFilePath();
									Path filePath = Paths.get(list.getUpFilePath());
				...
				for (MultipartFile multipartFile : multipartFiles) {
					...
					if(!multipartFile.isEmpty()) {  // 파일 있을때 
						...
						try {
							...
							//파일 domain 생성 
							StoreFileDomain storeFileDomain = StoreFileDomain.builder()
									.stSeq(stSeq)
									.mbId(mbId)
									.upOriginalFileName(origFilename)
									.upNewFileName("resources/upload/"+newFileName) // WebConfig에 동적 이미지 폴더 생성 했기때문
								...
								// db 인서트
								uploadMapper.stfileUpload(storeFileDomain);
								System.out.println("upload done");
	...
	public void stContentRemove(HashMap<String, Object> map) {
		uploadMapper.stContentRemove(map);
	}
	@Override
	...
	public void stFileRemove(StoreFileDomain storeFileDomain) {
		uploadMapper.stFileRemove(storeFileDomain);
	}
